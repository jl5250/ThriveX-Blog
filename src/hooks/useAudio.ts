...
export default function useAudio(): IAudio {
  const { surgeMusicList, hotMusicList, newMusicList, originalMusicList, dailyMusicList, currentMusicType, duration, currentMusic, currentTime } = useMusicStore();
  // 是否播放音乐
  ...
  const switchMusic = useCallback(
      ...
      if (currentMusicList.length) {
        ...
        switch (order) {
          case 'cycle': {
            ...
            currentIndex = currentIndex < 0 ? currentMusicList.length - 1 : currentIndex % currentMusicList.length;
    ...
    [currentMusicType, currentMusic, isMusic, surgeMusicList, hotMusicList, newMusicList, originalMusicList, dailyMusicList]
  );
