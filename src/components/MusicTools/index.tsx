...
import { FaVolumeHigh, FaVolumeLow, FaVolumeOff, FaVolumeXmark, FaForwardStep, FaBackwardStep, FaPause, FaPlay, FaBarsStaggered, FaRepeat, FaShuffle } from 'react-icons/fa6';
...
export default function MusicTools() {
  ...
  const { isMusic, audioRef, currentOrder, volume, currentTime, duration, changeJingyin, canplay, onEnd, onError, switchMusicStaus, switchMusic, setVolume } = audioInfo;
  ...
  return (
    <>
      <audio ref={audioRef} src={url} aria-label="music-player" onTimeUpdate={(e) => lyricInfo.updateTime(e)} onCanPlay={(e) => canplay(e)} onEnded={() => onEnd()} onError={() => onError()} className="hidden" />
      ...
      <Card shadow="lg" isFooterBlurred className={`fixed bottom-[5%] left-[1%] md:top-[68%] md:left-[1%] border-none z-[998] bg-white/80 dark:bg-neutral-900/80 backdrop-blur-xl w-full transition-all duration-500 ease-in-out ${active ? 'translate-x-0 translate-y-0 scale-100 opacity-100' : 'translate-x-[-500px] translate-y-[500px] scale-0 opacity-0'} ${isExpanded ? 'h-[85vh] md:w-[70vw] md:h-[80vh] md:top-[8%]' : 'h-[295px] md:w-[700px] md:h-[200px]'}`}>
        ...
        {/* 音乐信息组件 */}
        <CardFooter className={`absolute z-10 bottom-0 grid gap-1 grid-flow-row-dense grid-rows-4 grid-cols-12 items-center justify-center w-full h-[200px] transition-all duration-500 ease-in-out transform ${isExpanded ? 'md:gap-x-10' : 'md:gap-x-2'}`}>
          {/* 歌词 */}
          <div className={`flex flex-col items-center row-span-2 md:row-span-2 row-start-1 md:row-start-2 col-span-12 col-start-1 h-full md:h-[180px] ${isExpanded ? 'md:col-start-2 md:col-span-4' : 'md:col-start-2 md:col-span-5'}`}>
