...
const ProfileCardComponent: React.FC<ProfileCardProps> = ({ theme, avatarUrl, iconUrl, grainUrl, innerGradient, behindGlowEnabled = true, behindGlowColor, behindGlowSize, className = '', enableTilt = true, enableMobileTilt = false, mobileTiltSensitivity = 5, miniAvatarUrl, name = 'Javi A. Torres', title = 'Software Engineer', showUserInfo = true }) => {
  ...
  return (
    <div ref={wrapRef} className={`flex flex-col items-center bg-no-repeat mb-5 ${className}`.trim()} style={{ perspective: '500px', transform: 'translate3d(0, 0, 0.1px)', ...cardStyle } as React.CSSProperties}>
      {behindGlowEnabled && (
        <div
      ...
      <div ref={shellRef} className="relative z-[1] group">
        ...
        >
          ...
          >
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col items-start gap-1.5">
                      <div className="flex justify-evenly w-[100%] mx-auto pt-6">
                        {socialList?.map((item: Social, index: number) => (
                          <a key={index} href={item?.url} target="_blank" rel="noopener noreferrer">
                            <Image src={getIcon(item?.name)} alt={item?.name} title={item?.name} className="w-[26px] h-[23px]" />
                          </a>
                        ))}
                      </div>
                    </div>
            ...
            >
              <div className="w-full absolute flex flex-col" style={{ top: '1em', display: 'flex', gridArea: 'auto' }}>
                <h3
