...
const BentoCardGrid: React.FC<{
  ...
  <div className="bento-section grid gap-2 p-3 max-w-[55rem] select-none relative" style={{ fontSize: 'clamp(1rem, 0.9rem + 0.5vw, 1.5rem)' }} ref={gridRef}>
    {children}
  </div>
);
              ...
              return (
                <ParticleCard key={index} className={baseClassName} style={cardStyle} disableAnimations={shouldDisableAnimations} particleCount={particleCount} glowColor={glowColor} enableTilt={enableTilt} clickEffect={clickEffect} enableMagnetism={enableMagnetism}>
                  <div className="card__header flex justify-between gap-3 relative text-white">
                    <Image src={card.label} alt={card.description} width={48} height={48} />
                  </div>
              ...
              >
                {card.template ? (
                  ...
                  <>
                    ...
                    <div className="card__content flex flex-col relative text-white">
                      <h3 className={`card__title font-normal text-6xl m-0 mb-1 ${textAutoHide ? 'text-clamp-1' : ''} ${card.textColor}`}>{card.title}</h3>
                      <p className={`card__description text-sm leading-5 opacity-90 ${textAutoHide ? 'text-clamp-2' : ''} ${card.textColor}`}>{card.description}</p>
                    </div>
                  </>
                )}
              </div>
